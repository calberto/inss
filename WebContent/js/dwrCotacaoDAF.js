...
function carregaComboTipoDocumento(cdCategoria){

if(cdCategoria == ""){
   vform = document.forms[0];
   vform.cdTipoDocumento.value = "";
   vform.cdTipoDocumento.disabled= true;
   
}else{
	FacadeAjax.carregaComboTipoDocmentoRic(cdCategoria, 
	                                    {callback:function(listBean) {
	                                  		retornoCarregaComboTipoDoc(listBean);
	                                  	}
                                     });
...
function retornoCarregaComboTipoDoc(listBean){

   DWRUtil.removeAllOptions("cdTipoDocumento");
   DWRUtil.addOptions("cdTipoDocumento", listBean, "valorOption", "nomeOption");
    
   vform = document.forms[0];
   vform.cdTipoDocumento.disabled= false;
